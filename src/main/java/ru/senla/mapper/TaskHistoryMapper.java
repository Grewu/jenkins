...
/**
 * Mapper interface for converting between {@link TaskHistory} entities and their corresponding
 * DTOs, {@link TaskHistoryRequest} and {@link TaskHistoryResponse}.
 *
 * <p>This interface uses MapStruct to generate the implementation for mapping properties between
 * the entities and DTOs. It is annotated with {@code @Mapper(componentModel = "spring")} to enable
...
public interface TaskHistoryMapper {
  ...
  @Mapping(
      target = "assignedTo",
      expression = "java(new UserProfile(taskHistoryRequest.assignedTo()))")
  @Mapping(target = "project", expression = "java(new Project(taskHistoryRequest.project()))")
  @Mapping(
      target = "createdBy",
      expression = "java(new UserProfile(taskHistoryRequest.createdBy()))")
  @Mapping(
      target = "changedBy",
      expression = "java(new UserProfile(taskHistoryRequest.changedBy()))")
  ...
  /**
   * Updates an existing {@link TaskHistory} entity with values from a {@link TaskHistoryRequest}.
   *
