...
/**
 * The UserProfileRestController class provides REST endpoints for managing user profiles. This
 * controller supports creating, retrieving, updating, and deleting user profiles, as well as
 * retrieving comments associated with a specific profile. Each endpoint has specific authorization
 * requirements and supports JSON responses. The {@link UserProfileService} is used to handle the
 * business logic related to user profile management.
 *
 * <p>Endpoints:
 *
 * <ul>
 *   <li><b>POST /api/v0/user_profiles</b>: Creates a new user profile.
 *   <li><b>GET /api/v0/user_profiles</b>: Retrieves a paginated list of all user profiles.
 *   <li><b>GET /api/v0/user_profiles/{id}</b>: Retrieves a specific user profile by its ID.
 *   <li><b>GET /api/v0/user_profiles/{userProfileId}/comments</b>: Retrieves comments associated
 *       with a user profile by its ID.
 *   <li><b>PUT /api/v0/user_profiles/{id}</b>: Updates an existing user profile with the specified
 *       ID.
 *   <li><b>DELETE /api/v0/user_profiles/{id}</b>: Deletes the user profile with the specified ID.
 * </ul>
 *
 * <p>Authorization requirements:
 *
 * <ul>
 *   <li><b>user_profile:read</b> permission is required to retrieve and view user profiles and
 *       comments.
 *   <li><b>user_profile:write</b> permission is required to create and update user profiles.
 *   <li><b>user_profile:delete</b> permission is required to delete user profiles.
 *   <li><b>comments:read</b> permission is required to retrieve comments.
 * </ul>
 *
...
public class UserProfileRestController {
   ...
   *
   * @param userProfileRequest the {@link UserProfileRequest} object containing the user profile
   *     details.
   * @return a {@link ResponseEntity} with the created {@link UserProfileResponse} and HTTP status
   *     201 (Created).
   */
  @PostMapping
   ...
   * @return a {@link ResponseEntity} containing a paginated list of {@link CommentResponse} and
   *     HTTP status 200 (OK).
   */
  @GetMapping("/{userProfileId}/comments")
  @PreAuthorize("hasAuthority('user_profile:read') && hasAuthority('comments:read')")
  public ResponseEntity<Page<CommentResponse>> getCommentsByProfileId(
      @PathVariable Long userProfileId, @PageableDefault(20) Pageable pageable) {
   ...
   *
   * @param pageable pagination information, with a default page size of 20.
   * @return a {@link ResponseEntity} containing a paginated list of {@link UserProfileResponse} and
   *     HTTP status 200 (OK).
   */
  @GetMapping
   ...
   * @return a {@link ResponseEntity} containing the updated {@link UserProfileResponse} and HTTP
   *     status 200 (OK).
   */
  @PutMapping("/{id}")
  @PreAuthorize("hasAuthority('user_profile:write')")
  public ResponseEntity<UserProfileResponse> update(
      @PathVariable Long id, @RequestBody UserProfileRequest userProfileRequest) {
    return ResponseEntity.status(HttpStatus.OK)
        .contentType(MediaType.APPLICATION_JSON)
