...
/**
 * The TaskRestController class provides REST endpoints for managing tasks. This controller supports
 * creating, retrieving, updating, and deleting tasks, as well as filtering tasks and retrieving
 * task history. Each endpoint has specific authorization requirements and supports JSON responses.
 * It uses {@link TaskService} to handle the business logic related to task management.
 *
 * <p>Endpoints:
 *
 * <ul>
 *   <li><b>POST /api/v0/tasks</b>: Creates a new task.
 *   <li><b>GET /api/v0/tasks</b>: Retrieves a paginated list of all tasks.
 *   <li><b>GET /api/v0/tasks/filter</b>: Retrieves a paginated list of tasks based on filter
 *       criteria.
 *   <li><b>GET /api/v0/tasks/{taskId}/history</b>: Retrieves the history of a specific task by task
 *       ID.
 *   <li><b>GET /api/v0/tasks/{id}</b>: Retrieves a specific task by its ID.
 *   <li><b>PUT /api/v0/tasks/{id}</b>: Updates a specific task by its ID.
 *   <li><b>DELETE /api/v0/tasks/{id}</b>: Deletes a specific task by its ID.
 * </ul>
 *
 * <p>Authorization Requirements:
 *
 * <ul>
 *   <li><b>task:read</b> permission is required for retrieving and viewing tasks and task history.
 *   <li><b>task:write</b> permission is required for creating and updating tasks.
 *   <li><b>task:delete</b> permission is required for deleting tasks.
 *   <li><b>task_history:read</b> permission is required in combination with <b>task:read</b> to
...
public class TaskRestController {
   ...
   * @return a {@link ResponseEntity} containing the created {@link TaskResponse} with HTTP status
   *     201 (Created).
   */
  @PostMapping
  ...
  @GetMapping("/filter")
  @PreAuthorize("hasAuthority('task:read')")
  public ResponseEntity<Page<TaskResponse>> getAllByFilter(
      @Valid TaskFilter taskFilter, @PageableDefault(20) Pageable pageable) {
    return ResponseEntity.status(HttpStatus.OK)
        .contentType(MediaType.APPLICATION_JSON)
  ...
  public ResponseEntity<Page<TaskHistoryResponse>> getAllTaskHistoryByTaskId(
      @PathVariable Long taskId, @PageableDefault(20) Pageable pageable) {
    return ResponseEntity.status(HttpStatus.OK)
        .contentType(MediaType.APPLICATION_JSON)
   ...
   * @return a {@link ResponseEntity} containing the updated {@link TaskResponse} with HTTP status
   *     200 (OK).
   */
  @PutMapping("/{id}")
  @PreAuthorize("hasAuthority('task:write')")
