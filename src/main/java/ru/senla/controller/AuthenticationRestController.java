...
/**
 * The AuthenticationRestController class provides REST endpoints for user authentication and
 * registration. This controller handles requests for user registration and authentication,
 * returning either an authorization token or a user response object based on the request type. It
 * is annotated with custom logging and validation annotations and is integrated with the {@link
 * UserService} to process authentication-related operations.
 *
 * <p>Endpoints:
 *
 * <ul>
 *   <li><b>POST /api/v0/auth/register</b>: Registers a new user and returns an authorization token.
 *   <li><b>POST /api/v0/auth/authenticate</b>: Authenticates an existing user and returns user
 *       details.
 * </ul>
 *
 * <p>Request format for both endpoints expects a {@link UserRequest} payload in JSON format.
 *
 * @see UserRequest
 * @see UserResponse
 * @see UserService
